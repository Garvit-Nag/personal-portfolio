      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-12 lg:gap-16 items-stretch">
          ...
          <div className="flex flex-col">
            ...
            <div className="border border-[rgba(255,255,255,0.08)] rounded-xl p-4 sm:p-6 flex-1">
              <StaggerContainer>
                {EXPERIENCES.map((exp, index) => (
                  <StaggerItem key={index}>
                    <div className={`flex gap-4 sm:gap-5 py-6 ${index < EXPERIENCES.length - 1 ? 'border-b border-[rgba(255,255,255,0.06)]' : ''}`}>
                      {/* Icon + Year */}
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-sans font-semibold text-[15px] sm:text-[16px] text-[#e2e2e2] capitalize">
                          {exp.title}
                        </h3>
                        <p className="font-mono text-[10px] tracking-widest text-[#7a7a7a] mt-1 uppercase">
                          {exp.subtitle}
                        </p>
                        ...
                        {exp.link && (
                          <Link
                            href={exp.link.href}
                            className="inline-flex items-center gap-2 font-mono text-[10px] tracking-widest text-[#c8c8c8] border border-[rgba(255,255,255,0.12)] rounded-lg px-3 py-2 mt-4 hover:border-[rgba(255,255,255,0.3)] hover:bg-[rgba(255,255,255,0.04)] transition-all duration-180"
                          >
                            {exp.link.text}
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="relative flex-1 flex flex-col h-full">
              {/* Connecting line */}
              <div className="absolute left-[11px] top-3 bottom-6 w-px bg-linear-to-b from-[rgba(255,255,255,0.15)] via-[rgba(255,255,255,0.06)] to-transparent" />
              ...
              <div className="flex flex-col justify-between flex-1 h-full gap-5">
                {EDUCATION.map((edu, index) => (
                  <div key={index} className="flex gap-4 sm:gap-5 relative group flex-1">
                    ...
                    {/* Content card */}
                    <div className="flex-1 flex flex-col bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] rounded-lg p-4 sm:p-5 transition-all duration-300 group-hover:bg-[rgba(255,255,255,0.04)] group-hover:border-[rgba(255,255,255,0.12)]">
                      <div className="mb-2">
                        <p className="font-mono text-[10px] tracking-widest text-[#5a5a5a] uppercase mb-1.5">
                          {edu.period}
                        </p>
                        <h3 className="font-sans font-semibold text-[13px] sm:text-[14px] text-[#d8d8d8] leading-[1.4]">
                          {edu.degree}
                        </h3>
                      </div>
                      ...
                      <div className="mt-auto">
                        <span className="inline-block shrink-0 font-mono text-[10px] tracking-widest text-[#5a5a5a] border border-[rgba(255,255,255,0.08)] rounded px-2 py-0.5">
                          {edu.grade}
                        </span>
                      </div>
                    </div>
